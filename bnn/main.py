...
parser.add_argument('--bin_input', type=int, default=1, help='input binary')
parser.add_argument('--drop_rate',
                    type=float,
                    default=0.0,
                    help='how often to drop')
parser.add_argument('--dropc_keep_prob',
                    type=float,
                    default=None,
                    help='fraction of inputs to use when computing thresholds')
parser.add_argument('--n_agg',
                    type=int,
                    default=1,
...
if args.data == 'cancer':
    if args.bin_input:
        cancer_tr = load_data.NPZDataset('data/cancer/cancer_train.npz',
                                         2,
                                         0,
                                         as_int=False,
                                         divisions=args.divs)
        cancer_te = load_data.NPZDataset('data/cancer/cancer_test.npz',
                                         2,
                                         1,
                                         mins=cancer_tr.mins,
    ...
    tr = load_data.NPZDataset2D('data/faces/faces_train.npz',
                                1,
                                0,
                                as_int=False)
    te = load_data.NPZDataset2D('data/faces/faces_test.npz',
                                1,
                                1,
                                mean=tr.mean,
                                std=tr.std,
    ...
    if args.bin_input:
        diabetes_tr = load_data.NPZDataset('data/diabetes/diabetes_train.npz',
                                           3,
                                           0,
                                           divisions=args.divs)
        diabetes_te = load_data.NPZDataset('data/diabetes/diabetes_test.npz',
                                           3,
                                           1,
        ...
        diabetes_te = load_data.NPZDataset('data/diabetes/diabetes_test.npz',
                                           ...
                                           as_int=True,
                                           divisions=args.divs)
    train_loader = torch.utils.data.DataLoader(diabetes_tr,
                                               batch_size=args.batch_size,
                                               shuffle=True,
                                               **kwargs)
    test_loader = torch.utils.data.DataLoader(diabetes_te,
                                              batch_size=args.batch_size,
                                              shuffle=True,
                                              **kwargs)
...
def train(epoch):
    ...
    for batch_idx, (data, target) in tqdm(enumerate(train_loader)):
        ...
        correct += pred.eq(target.data.view_as(pred).type(
            pred.type())).cpu().sum()
        #if args.data == 'mnist':
        #    if epoch%10==0:
    ...
    if args.data == 'mnist':
        print('\nValid set: Accuracy: {}/{} ({:.0f}%)\n'.format(
            val_correct, 10000.0, 100. * val_correct / 10000.0))
    return (correct / (len(train_loader.dataset) * 1.0)), (val_correct /
                                                           10000.0)
...
def test():
    ...
    for data, target in test_loader:
        ...
        correct += pred2.eq(target.data.view_as(pred2).type(
            pred2.type())).cpu().sum()
